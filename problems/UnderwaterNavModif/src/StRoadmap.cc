...
void StRoadmap::insertGoalMilestones(vector<StateVals> &goals, long nGoalsSamp) {
	long nGoals = goals.size();
	//for (long i = 0; i < nGoalsSamp; i++) {
	vector<StateVals>::iterator itV;
	bool inserted;
	while (nVerts < nGoalsSamp) {
		long rawIdx = (long) floor(GlobalResources::randGen.ranf_arr_next()*nGoals);
		StateVals st(2); st[0] = goals[rawIdx][0]; st[1] = goals[rawIdx][1];
		inserted = true;
		...
		if (inserted) { V.push_back(st); nVerts ++; }
	}
...
void StRoadmap::sampleAMilestone(StateVals &st) {
	long rawIdx = (long) floor(GlobalResources::randGen.ranf_arr_next()*totW);
...
bool StRoadmap::insertMilestone(StateVals &st) {
	...
	long nTry = 0;
	for (itV = V.begin(), i = 0; itV != V.end(); itV++, i++) {
		if (dist(st, *itV) < maxDistCon && nTry < maxTryCon && (c = lineSegOk(st, *itV)) > -1) {
...
//cerr << "About to erase " << itNxt->first << " of v " << i << endl;
			...
			ptrToIdx[itNxt->first] = q.insert(pair<long, long>(itNxt->second, itNxt->first));
		...
		while (!q.empty()) {
			itQ = q.begin();
			currIdx = itQ->second;
			for (itNxt = inEdges[currIdx].begin(); itNxt != inEdges[currIdx].end(); itNxt++) {
				if (!visited[itNxt->first] && (tmpDist = itQ->first + itNxt->second) < ptrToIdx[itNxt->first]->first) {
					q.erase(ptrToIdx[itNxt->first]);
					ptrToIdx[itNxt->first] = q.insert(pair<long, long>(tmpDist, itNxt->first));
...
*/
	...
	for (itVecVecL->begin(); itVecVecL != allDist.end(); itVecVecL++) {
		long i = 0;
		for (itVecL = itVecVecL->begin(); itVecL != itVecVecL->end(); itVecL++, i++) {
...
void StRoadmap::updateRoadmap(map< long, map<long, short> > &env_, vector<StateVals> &goals, long nGoalsSamp) {
	env = env_;
...
bool StRoadmap::VContains(long x, long y) {
	StateVals st(2); st[0] = x; st[1] = y;
	vector<StateVals>::iterator itV;
	for (itV = V.begin(); itV != V.end(); itV++) {
		if (dist(st, *itV) == 0) { return true; }
	}
	return false;
}
			...
			if (itCellType != env.end()) {
				if (itCellType->second.find(y) != itCellType->second.end()) {
					...
					switch(env[x][y]) {
						...
						case 3: { os << "O"; break; }
						case 5: { os << "W"; break; }
						default: {os << env[x][y] ; break; }
					}
				}
				else if (VContains(x, y)) { os << "V"; }
				else { os << " "; }
			}
			else if (VContains(x, y)) { os << "V"; }
			else { os << " "; }
