...
/*
long RockSampleModel::getRolloutAct(StateVals &stVals) {
	double tmp = GlobalResources::randGen.ranf_arr_next();
...
long RockSampleModel::getRolloutAct(StateVals &stVals, double *rew) {
	long selectedActIdx;
	double tmp = GlobalResources::randGen.ranf_arr_next();
	if (tmp < rolloutExploreTh) {	// Random
		tmp = GlobalResources::randGen.ranf_arr_next();
...
double RockSampleModel::getDistToNearestGoal(StateVals &st) {
...
double RockSampleModel::getDistToNearestObs(StateVals &st, StateVals &nxtSt) {
...
/*
double RockSampleModel::getExpDist(StateVals &s, long firstAct) {
	double totDist = 0.0;
...
double RockSampleModel::getDist(StateVals &s1, StateVals &s2) {
	double d = 0.0;
...
bool RockSampleModel::getNextState(StateVals &sVals, long actIdx, StateVals &nxtSVals, ObsVals &obs) {
	nxtSVals.clear();
//cerr << "input currSt " << sVals[0] << " " << sVals[1] << " a " << actIdx << endl;	
	getNextState(sVals, actIdx, nxtSVals);
//cerr << "currSt " << sVals[0] << " " << sVals[1] << " a " << actIdx << " nxtSt: " << nxtSVals[0] << " " << nxtSVals[1] << endl;	
...
void RockSampleModel::inObsRegion(StateVals &sVals, ObsVals &obs) {
	obs.resize(2);
/*
...
bool RockSampleModel::inGoal(StateVals &st) {
...
bool RockSampleModel::inRock(StateVals &st) {
...
/*
bool RockSampleModel::inSpcRew(StateVals &st) {
	vector<StateVals>::iterator it;
...
bool RockSampleModel::getNextState(StateVals &currStVals, long actIdx, double *immediateRew, StateVals &nxtSVals, ObsVals &obs) {
	bool isTerm;
	getNextState(currStVals, actIdx, nxtSVals);
	inObsRegion(nxtSVals, obs);
//cerr << "GettingNxtState from " << currStVals[0] << " " << currStVals[1] << " act " << actIdx << endl;	
	*immediateRew = getReward(currStVals, actIdx);
...
vector<StateVals>::iterator RockSampleModel::getIterator(vector<StateVals> &vecStVals, long x, long y) {
	vector<StateVals>::iterator it;
...
void RockSampleModel::getReachableSt(StateVals &s, long actId, vector<StateVals> &nxtS) {
	nxtS.clear();
	switch(actId) {
		case EAST: {
			StateVals sp(2); 
...
int RockSampleModel::findCollision(StateVals &s) {
	int i;
